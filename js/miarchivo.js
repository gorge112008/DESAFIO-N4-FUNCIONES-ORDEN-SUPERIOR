...
function Menu(Nombre, Stock, Costo, Descripción, Total) {
  ...
  this.agotado = () => {
    alert(
      "Disculpe, no queda más " +
        this.Nombre +
        ", por favor elija otra opción del menú."
    );
  };
...
do {
  ...
  if (tm == null) {
    tm = 3;
  } //SE CANCELA EL MENU ESPECIAL Y SE INGRESA A TODO EL MENU DISPONIBLE.
  switch (parseInt(tm)) {
...
let ingreso = prompt(
  "Buen día, selecciona la opción del menú que desea pedir:       \n" +
    ...
    "0-Ninguna, Salir                                   \n" +
    "***************************************************************\n" +
    "Si desea ver la carta detallada ingrese el " +
    (tamaño + 1) +
    "."
);
...
if (todototal == "0") {
  ...
  const msjfinal = pedido.reduce(
    (acum, ele) =>
      acum + "\n" + "*" + ele.Nombre + "= " + ele.total + " unidad(es).",
    "Su pedido es:"
  );
  //MOSTRANDO MENSAJE FINAL CONSTRUIDO.
  alert(
    msjfinal +
      "\n" +
      "El monto total de su pedido es de: S/." +
      todototal +
      " Soles.        \n" +
      "Su pedido estará listo en un tiempo aproximado de " +
      tiempo +
      " minutos       \n" +
      "Muchas Gracias por su compra, que tenga un buen día!!!"
  ); //ALERTA EMITIDA AL COMPLETAR CON ÉXITO EL PEDIDO.
}
